...
const addRaffel = async (req, res) => {
  try {
    ...
    const checkData = await SellerWallet.findOne({
    ...
    if (checkData)
      return res.status(HTTP.SUCCESS).send({
        ...
        message: "Wallet already exists with same token address!",
        data: {},
      });
...
const createRaffel = async (req, res) => {
  try {
    ...
    const check = await Raffel.findOne({
      $and: [{ token_address }, { wallet_address }],
    });
    if (check)
      return res.status(HTTP.SUCCESS).send({
        status: false,
...
const getinfo = async (req, res) => {
  try {
    ...
    const mergeById = (array1, array2) =>
      array1.map((itm) => ({
        ...array2.find(
          (item) => item.token_address === itm.token_address && item
        ),
        ...itm,
      }));
...
const buyTicket = async (req, res) => {
  try {
    ...
    if (!buyers_wallet || !token_address || !quantity)
      return res.status(HTTP.SUCCESS).send({
        status: true,
    ...
    const endpoint =
      "https://public-api-test.solscan.io/account/solTransfers?account=" +
      buyers_wallet +
      "&limit=1&offset=0";
    ...
    axios
      ...
      .then(async (response) => {
        ...
        for (const data of transactionList) {
          ...
          if (data.status == "Success") {
            ...
            const txnData = await new Transaction({
              buyers_wallet,
              src: data.src,
              txHash: data.txHash,
              blockTime: data.blockTime,
              totalTickes: quantity,
            }).save();
            if (!txnData)
              return res.status(HTTP.BAD_REQUEST).send({
                status: false,
                code: HTTP.NOT_ALLOWED,
                message: "could not add txn to db",
                data: {},
              });
          }
        }
      ...
      .catch((error) => {
        ...
        return res.status(HTTP.BAD_REQUEST).send({
          status: false,
          code: HTTP.NOT_ALLOWED,
          message: "could not find wallet address",
          data: {},
        });
      });
    ...
    for (const data of checkSeller) {
      if (buyers_wallet === data.wallet)
        return res.status(HTTP.SUCCESS).send({
          status: true,
          code: HTTP.NOT_ALLOWED,
          message: "seller cannot buy its own ticket!",
          data: {},
        });
    }
    ...
    for (const data of checkQuantity) {
      exceedAmt = data.ticket_supply;
    }

    for (const data of checkQuantity) {
      if (quantity > data.ticket_remaining || quantity > exceedAmt)
        return res.status(HTTP.SUCCESS).send({
          status: true,
          code: HTTP.NOT_ALLOWED,
          message: "quantity is exceding!",
          data: {},
        });
      ...
      const update = await Raffel.findOneAndUpdate(
        { token_address },
      ...
      if (!update)
        return res.status(HTTP.SUCCESS).send({
          status: true,
          code: HTTP.NOT_ALLOWED,
          message: "could not update ticket remaining!",
          data: {},
        });
    ...
    const already_bought = await Participant.find({
      buyers_wallet,
      token_address,
    });
    for (const data of already_bought) {
      ...
      let update_quantity = await Participant.findOneAndUpdate(
        { _id: data._id },
    ...
    const addParticipant = await new Participant({
      buyers_wallet,
      price,
      quantity,
      token_address,
    ...
    if (!addParticipant)
      return res.status(HTTP.SUCCESS).send({
        status: true,
        code: HTTP.NOT_ALLOWED,
        message: "could not add the current participant!",
        data: {},
      });
...
const checkRemaining = async (req, res) => {
  try {
    ...
    for (const data of tickets) {
      if (data.ticket_remaining > 0)
        return res.status(HTTP.SUCCESS).send({
          status: false,
          code: HTTP.NOT_ALLOWED,
          message: "still tickets remaining",
          data: {},
        });
    }
...
const participants = async (req, res) => {
  try {
    ...
    const buyers = await Participant.find(
      { token_address },
      { _id: 0, __v: 0, token_address: 0 }
    ).sort({ quantity: -1 });
    let result = [];
...
const ticketRemaining = async (req, res) => {
  try {
    const { token_address } = req.body;
    const data = await Raffel.find(
      { token_address },
      { ticket_remaining: 1, _id: 0 }
    );
...
const getAllData = async (req, res) => {
  try {
    ...
    for (const data of wallet) {
      ...
      for (const d of raffel) {
        result.push({
          ...
          raffel_end_date: d.raffel_end_date,
          url: d.url,
          ticket_supply: d.ticket_supply,
          ticket_remaining: d.ticket_remaining,
    ...
    return res
      .status(HTTP.SUCCESS)
      .send({ status: true, code: HTTP.SUCCESS, message: "all data", result });
  } catch (err) {
    console.log(err);
    return res.status(HTTP.SUCCESS).send({
      status: false,
...
const getSingleData = async (req, res) => {
  try {
    ...
    const wallet = await SellerWallet.find({ token_address });
    // const raffel = await Raffel.find({})
    ...
    return res
      .status(HTTP.SUCCESS)
      .send({ status: true, code: HTTP.SUCCESS, message: " data", result });
  } catch (err) {
    console.log(err);
    return res.status(HTTP.SUCCESS).send({
      status: false,
  ...
  try {
    ...
    if (check)
      return res.status(HTTP.SUCCESS).send({
        status: false,
        code: HTTP.BAD_REQUEST,
        message: "winner already generated.",
        data: {},
      });
    ...
    if (!random)
      return res.status(HTTP.SUCCESS).send({
        status: false,
        code: HTTP.BAD_REQUEST,
        message: "something went wrong while getting winner.",
        data: {},
      });

    // save winner to model
    for (const data of random) {
      const saveWinner = await new Winner({
        buyers_wallet: data.buyers_wallet,
        token_address: data.token_address,
      }).save();
    ...
    const sellerWalletUpdate = await SellerWallet.findOneAndUpdate(
      { token_address },
      { status: false }
    );
    if (!sellerWalletUpdate)
      return res.status(HTTP.SUCCESS).send({
        status: false,
        code: HTTP.BAD_REQUEST,
        message: "could not update seller wallet.",
        data: {},
      });
